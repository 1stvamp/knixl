...
impl Module for Backups {
    ...
    fn lower(&self, node: &KdlNode, ctx: &mut LowerCtx) -> Result<LowerOutput, LowerError> {
        ...
        let repo =
            child_arg_str(node, "repo").ok_or_else(|| LowerError::missing("backups.repo"))?;
        ...
        set.insert(
            AttrKey::Ident("timerConfig".into()),
            NixExpr::AttrSet(timer),
        );
        set.insert(
        ...
        Ok(LowerOutput::units(vec![Unit {
            bucket: Bucket::Named("backup".into()),
            assignment,
            module: String::new(),
        }]))
    }
...
fn schema() -> NodeSchema {
    NodeSchema {
        ...
        children: vec![
            ...
            leaf(
                "schedule",
                "A systemd OnCalendar expression (default: daily).",
                false,
            ),
            leaf(
                "when",
                "A raw Nix condition; the backup is wrapped in lib.mkIf.",
                false,
            ),
        ],
        open_children: false,
...
mod tests {
    ...
    fn backups_emits_a_named_side_file_gated_by_when() {
        ...
        assert!(
            matches!(&a.condition, Some(NixExpr::Raw(r)) if r.src == "config.services.postgresql.enable")
        );
