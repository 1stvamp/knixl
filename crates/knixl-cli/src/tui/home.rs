...
impl HomeModel {
    ...
    fn route_for(label: &str) -> &'static str {
        ITEMS.iter().find(|(l, _)| *l == label).map(|(_, k)| *k).unwrap_or("quit")
    }
    ...
    pub fn update(&mut self, msg: Msg, size: (u16, u16)) -> Step {
        ...
        match code {
            KeyCode::Enter => {
                let route = self.list.selected_item().map(|i| Self::route_for(&i.title)).unwrap_or("quit");
                match route {
    ...
    pub fn view(&self, size: (u16, u16)) -> String {
        ...
        format!(
            ...
            panel,
            widgets::footer(&[("\u{2191}/\u{2193}", "move"), ("enter", "select"), ("q", "quit")]),
        )
    }
}
